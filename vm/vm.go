...
type VM struct {
	...
	stack       []object.Object
	sp          int // 始终指向栈顶的下一个空位置。栈顶的值是 stack[sp-1]
...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpHash:
			...
			hash, err := vm.buildHash(vm.sp-numPairs, vm.sp)
			if err != nil {
				return err
			}
		...
		case code.OpGetFree:
			...
			vm.currentFrame().ip += 1
			currentClosure := vm.currentFrame().fn
...
func isTruththy(obj object.Object) bool {
	switch obj := obj.(type) {
